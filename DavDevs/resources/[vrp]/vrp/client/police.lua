...
local cloroformio = false
function tvRP.toggleCloroformio()
	cloroformio = not capuz
	if cloroformio then
		tvRP.setDiv("cloroformio",".div_cloroformio { background: #000; margin: 0; width: 100%; height: 100%; }","")
		SetPedComponentVariation(PlayerPedId(),1,69,2,2)
	else
		tvRP.removeDiv("cloroformio")
		SetPedComponentVariation(PlayerPedId(),1,0,0,2)
	end
end

function tvRP.setCloroformio(flag)
	if capuz ~= flag then
		tvRP.toggleCapuz()
	end
end

function tvRP.isCloroformio()
	return capuz
end
